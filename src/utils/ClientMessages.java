package utils;

public interface ClientMessages {
    String NO_SUCH_USER = "Enter a unique loginview, please";
    String CORRECT_PASS = "Login and pass are correct";
    String INCORRECT_PASS = "Login or password is incorrect. Try again";
    String LOGIN_IS_OCCUPIED = "Login is occupied. Enter a unique login, please ";
    String TRANSITION_COMPLETE = "Files list received";
    String GET_FILES = "List of files received";
}
